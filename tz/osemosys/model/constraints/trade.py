...
def add_trade_constraints(
    ...
    con = (
        lex["TotalTradeCapacityAnnual"]
        * (1 + ds["TradeLossBetweenRegions"])
        >= m["Export"] / (ds["CapacityToActivityUnit"] * ds["YearSplit"])
    )
    ...
    con = (
        lex["TotalTradeCapacityAnnual"]
        * (1 + ds["TradeLossBetweenRegions"])
        >= m["Import"] / (ds["CapacityToActivityUnit"] * ds["YearSplit"])
    )
    m.add_constraints(con, name="TC1b_TradeConstraint_Import")
    
    con = (
        lex["NewTradeCapacity"]
        <= ds["TotalAnnualMaxTradeInvestment"] * ds["TradeRoute"])
    return m