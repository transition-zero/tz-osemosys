...
def add_re_targets_constraints(ds: xr.Dataset, m: Model) -> Model:
    ...
    #      sum(dims="TECHNOLOGY")) *
    #     ds["YearSplit"] * ds["RETagFuel"].fillna(0)
    #     - m["RETotalProductionOfTargetFuelAnnual"]
    #     == 0
    # )
    # mask = ds["RETagFuel"] == 1
    ...
    con = (
        ...
        * ds["YearSplit"]
        * ds["RETagFuel"].fillna(0)
    ) * ds["REMinProductionTarget"].fillna(0) - (
        (m["RateOfActivity"] * ds["OutputActivityRatio"] * ds["YearSplit"]).sum("TIMESLICE")
        * ds["RETagTechnology"].fillna(0)
    ) <= 0
    ...
    #          where(ds["OutputActivityRatio"].sum("MODE_OF_OPERATION") != 0).
    #          sum(dims="MODE_OF_OPERATION")).
    #         sum(dims="TECHNOLOGY"))).sum("TIMESLICE") -
    #        m["UseByTechnologyAnnual"]
    #        == 0)
